...
def main():
    ...
    parser.add_argument("--node_number", type=int, help="Node number (e.g., 0, 1, 2, ...)")
    ...
    parser.add_argument("--bootstrap_port", type=int, help="Bootstrap node port (required if node_number != 0)")
    parser.add_argument("--signal_port", type=int, help="Port to listen for signals")
    
    # New arguments for triggering signals
    parser.add_argument("--trigger", action="store_true", help="Trigger a signal instead of running experiments")
    parser.add_argument("--trigger_host", help="Host to send the trigger signal to")
    parser.add_argument("--trigger_port", type=int, help="Port to send the trigger signal to")
    ...
    if args.node_number is None or args.signal_port is None:
        print("Error: --node_number and --signal_port are required for running experiments")
        sys.exit(1)
