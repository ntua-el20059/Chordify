        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as temp_socket:
            ...
            try:
                ...
                conn.close()
                    ...
                    print("⏳ Timeout: No response received within the timeout period.")
                    self.close()
            print("Getting all necessary keys from predecossor")                  
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as temp_socket:
            temp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            temp_socket.bind(("0.0.0.0", 0))  # Bind to a free port
            temp_port = temp_socket.getsockname()[1]
            ...
            "sender_temp_port": temp_port,
            "sender_id": self.node_id,
            "finished": False  # Initialize found_predecessor as False
            }

            print(request)

            self.pass_request(request)
            print("🕒 Waiting for keys from successor...")
            try:
                conn, _ = temp_socket.accept()
                data = conn.recv(1024).decode()
                if data:
                    ...
                    if response["finished"] == True:
                         print("Got response from successor and waiting for his keys")
                conn.close()
            except socket.timeout:
                    print("⏳ Timeout: No response received within the timeout period.")
                    self.close()
